...
module.exports = function({ commitsPerDay, workdaysOnly, startDate, endDate }) {
  const commitDateList = createCommitDateList({
    ...
    startDate: parse('2015/06/18'),
    endDate: parse('2023/12/21')
  ...
  (async function() {
    ...
    const historyFolder = "casino-brige";
    ...
    for (const date of commitDateList) {
      ...
      await execAsync(`echo "${date}" > readme.txt`);
      await execAsync(`git add .`);
      await execAsync(`git commit --quiet --date "${date}" -m "first commit"`);
    }
...
function createCommitDateList({
  ...
  while (currentDate <= endDate) {
    if(currentDate >= parse('2015/12/23') && currentDate < parse('2016/01/18')){ currentDate = parse('2016/01/18') }
    if(currentDate >= parse('2016/12/24') && currentDate < parse('2017/01/20')){ currentDate = parse('2017/01/20') }
    if(currentDate >= parse('2017/12/20') && currentDate < parse('2018/01/16')){ currentDate = parse('2018/01/16') }
    if(currentDate >= parse('2018/12/19') && currentDate < parse('2019/01/09')){ currentDate = parse('2019/01/09') }
    if(currentDate >= parse('2019/12/10') && currentDate < parse('2020/01/03')){ currentDate = parse('2020/01/03') }
    if(currentDate >= parse('2020/12/24') && currentDate < parse('2021/01/06')){ currentDate = parse('2021/01/06') }
    if(currentDate >= parse('2021/12/22') && currentDate < parse('2022/01/12')){ currentDate = parse('2022/01/12') }
    if(currentDate >= parse('2022/12/20') && currentDate < parse('2023/01/16')){ currentDate = parse('2023/01/16') }
